...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module ExcelCoordinate.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain ExcelCoordinate.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "ExcelCoordinate.psm1"
        } #validPSM1
